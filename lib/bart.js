...
const Bart = {
    ...
    saveFile : (fileName) => {
        ...
        if(atom.config.get('Bart.checkDirectoryExistence'))
        {
            const helper = require('./helper');
            const parentDirectories = helper.getAllParentDirectories(currentRoot, fileName).reverse();

            //For some reason 'this' refrence scope is not present inside promise
            const progressPanelRef = this.progressPanel;
            const progressMsgRef = this.progressMsg;

            helper.checkAndCreateDirectories(parentDirectories, webdav).then(() => {
                webdav.post(fileName).then(() => {
                    progressPanelRef.hide();
                }, (err) => {
                    progressMsgRef.setError(err);
                    setTimeout(() => progressPanelRef.hide(), 10000);
                    atom.notifications.addError(err.toString());
                });
            }, (err) => {
                atom.notifications.addError("Unable to create directory on server " + err);
            })
        }
        else
        {
